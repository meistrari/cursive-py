...
from ..custom_types import CursiveAskOnToken
...
def process_openai_stream(
    ...
    if payload.functions:
        data['usage']['prompt_tokens'] += get_token_count_from_functions(payload.functions)
    ...
    for slice in response:
        ...
        for i in range(len(slice['choices'])):
            ...
            if delta and delta.get('function_call') and delta['function_call'].get('name'):
                data['choices'][i]['message']['function_call'] = delta['function_call']

            if delta and delta.get('function_call') and delta['function_call'].get('arguments'):
